...
import {TextInputPortLabel} from '../../CustomPorts/TextInputPortLabelWidget'
...
export class TestNodeWidget extends React.Component<TestNodeProps> {
	generatePort = (port:any) => {
        if(port){
		return <TextInputPortLabel engine={this.props.engine} port={port} key={port.getID()} node={this.props.node} />;
        }else{
            return <></>
        }
	};
	...
	render() {
		return (
			<S.Node
				data-default-node-name={"Test Number"}
